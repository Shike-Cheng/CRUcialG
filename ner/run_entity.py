...
from entity.utils import convert_dataset_to_samples, batchify, NpEncoder, ioc_reg
...
def reg_neridentity(batche, pred_ner):
    new_pred_ner = [[] for l in range(len(pred_ner))]
    sample_id = 0
    for sample, preds in zip(batche, pred_ner):
        span_id = 0
        new_preds = []
        for span in sample["spans"]:
            if sample["spans_label"][span_id] != preds[span_id]:
                token_span = ""
    ...
    for i in range(len(batches)):
        ...
        '''for sample, preds in zip(batches[i], pred_ner):
            ...
            for gold, pred in zip(sample['spans_label'], preds):
                #if sample['spans_label'] != 0:
                #span_id += 1
                #print("######################eval-gold, pred#################")
                # print(gold, pred)
                l_tot += 1
                if pred == gold:
                    l_cor += 1
                if pred != 0:
                    tot_pred += 1
            for key, val in sample['same_ner'].items():
                for i in key:
                    if preds[i] == val:
                        cor += 1
                        break'''
        #if epoch == 1:
            #print("this is 2 epoch!")
        for sample, preds in zip(batches[i], pred_ner):
            for gold, pred in zip(sample['spans_label'], preds):
                l_tot += 1
                if pred == gold:
            ...
            '''for ner, label in sample["sent_ner_list"].items():
                if epoch == 1:
                    print("this is 2 epoch!")
                ner_list = []
                for z in ner:
                ...
                for x in range(len(preds)):
                    if preds[x] != 0 and preds[x] == ner_label2id[label]:
                        span_list = []
                        for y in range(sample["spans"][x][0], sample["spans"][x][1] + 1):
                            span_list.append(y)
                        if list(set(span_list) & set(ner_list)):
                            cor += 1
                            break'''
            for spans_id in range(len(sample['spans_label'])):
                if sample['spans_label'][spans_id] != 0:
                    ner_list = []
                    for n in range(sample['spans'][spans_id][0], sample['spans'][spans_id][1] + 1):
                        ner_list.append(n)
                    for x in range(len(preds)):
                        if preds[x] != 0 and preds[x] == sample['spans_label'][spans_id]:
                            span_list = []
                            for y in range(sample["spans"][x][0], sample["spans"][x][1] + 1):
                                span_list.append(y)
                            if list(set(span_list) & set(ner_list)):
