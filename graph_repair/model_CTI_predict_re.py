...
from utils import save_one_mol, save_one_reward, Reasonable_judgment
...
class GraphFlowModel(nn.Module):
    ...
    def get_current_graph(self, f_nodes, f_nodes_name, f_edges, i_node, i_edges, b_nodes, b_nodes_name, b_edges):
        cut_node = len(f_nodes) - 1

        current_nodes = f_nodes + [i_node] + b_nodes
        current_nodes_name = f_nodes_name + [[i_node]] + b_nodes_name
        # f_nodes_name.append([i_node])
        # current_nodes_name = f_nodes_name + b_nodes_name
        current_edges = f_edges + i_edges
        for b_e in b_edges:
            if b_e[0] > cut_node:
                b_e[0] += 1
            if b_e[1] > cut_node:
                b_e[1] += 1
        current_edges += b_edges

        return current_nodes, current_edges, current_nodes_name

    def get_miss_stage(self, stages_index):
        miss_id = []
        stage_id = 0
    ...
    def generate(self, cur_node_data, cur_adj_data, free_id, free_node, original_graph, temperature=0.75, mute=False, max_atoms=100, cnt=None):
        """
        ...
        with torch.no_grad():
            ...
            cur_adj_features = torch.from_numpy(cur_adj_data).cuda()
            # print(cur_adj_features.size())
            cur_adj_features = cur_adj_features.view(1, 10, 100, 100)

            cur_node_features = torch.zeros([1, max_atoms, self.node_dim]).cuda()


            for i in range(max_atoms):
                type_tensor = torch.zeros([6]).cuda()
                if cur_node_data[i] != 0:
                    type_tensor[cur_node_data[i] - 1] = 1.0
                cur_node_features[0, i] = type_tensor

            cur_adj_features = cur_adj_features.to(torch.float32)
            ...
            if i == 0:
                ...
                feature_id = node_dict[free_node] - 1
                ...
                while not valid:
                    ...
                    if len(invalid_bond_type_set) < 9 and resample_edge <= 100:  # haven't sampled all possible bond type or is not stuck in the loop
                        ...
                        if self.dp:
                            ...
                            latent_edge = self.flow_core.reverse(cur_node_features, cur_adj_features, latent_edge,mode=1, edge_index=torch.Tensor([[j + start, i]]).long().cuda()).view(-1)
                        edge_discrete_id = torch.argmax(latent_edge).item()
                    ...
                    if edge_discrete_id == 9:
                        ...
                        if edge_list in reasonal_list:
                            ...
                            if check_flag == False:
                                ...
                                if ((" " + str(j+start) + " " + str(i) + " " + num2bond[edge_discrete_id]) or (" " + str(i) + " " + str(j+start) + " " + num2bond[edge_discrete_id])) not in (original_graph):
                                    if ([node1+1,node2+1,edge_discrete_id+1]) in reasonal_list_right:
                                        if ([node1+1,node2+1,edge_discrete_id+1] in [[1,1,6],[1,2,6],[2,1,6],[2,2,6],[1,1,4],[1,2,4],[2,1,4],[2,2,4],[1,1,7],[1,2,7],[2,1,7],[2,2,7]]) and (i > j+start):
                                            ...
                                            # graph_edges.append([j + start, i, edge_discrete_id + 1])
                                            increase_edges.append([j + start, i, edge_discrete_id + 1])
                                            ...
                                            # graph_edges.append([i, j + start, edge_discrete_id + 1])
                                            increase_edges.append([i, j + start, edge_discrete_id + 1])
                                ...
                                valid = True
                                is_connect = True
                            # print(edge_discrete_id, i, j + start)
                            cur_adj_features[0, edge_discrete_id, i, j + start] = 1.0
                            cur_adj_features[0, edge_discrete_id, j + start, i] = 1.0
                        else:
                            edge_cnt -= 1
